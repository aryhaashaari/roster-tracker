...
func NewPlayerUpdate(repo repositories.Playerer) ucase.UseCase {
	return &playerUpdate{repo: repo}
}

// Serve store player data
func (u *playerUpdate) Serve(dctx *appctx.Data) appctx.Response {
	var (
	...
	af, err := u.repo.Update(ctx, param.PlayerName, param.Position, param.Physique, param.StatsList, param.Id)
	if err != nil {
		tracer.SpanError(ctx, err)
