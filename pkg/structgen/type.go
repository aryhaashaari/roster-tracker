...
func generateSchema(sc []ColumnSchema, tableName string) Schema {
	...
	for i := 0; i < len(sc); i++ {
		...
		if !gType.Nullable {
			vRule = append(vRule, `validation.Required`)
		}
		...
		if util.InArray(gType.Type, []string{"time.Time", "*time.Time"}) {
			col.IsTime = true
		}

		if gType.Type == "string" {
			vRule = append(vRule,
				fmt.Sprintf(`validation.Length(1, %d)`, sc[i].CharacterMaximumLength.Int64),
	...
	if importValidator {
		data.ImportPackage = append(data.ImportPackage, fmt.Sprintf(`%s/internal/validator`, data.ModuleName))
	}
