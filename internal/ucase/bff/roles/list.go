...
func NewRoleList(repo repositories.Roler) ucase.UseCase {
	return &roleList{repo: repo}
}

// Serve Role list data
func (u *roleList) Serve(dctx *appctx.Data) appctx.Response {
	var (
		param presentations.RoleQuery
		ctx   = tracer.SpanStart(dctx.Request.Context(), "ucase.list_list")
		lf    = logger.NewFields(
			logger.EventName("roleList"),
		)
	)
	...
	logger.InfoWithContext(ctx, fmt.Sprintf("success fetch roles to database"), lf...)
	return *appctx.NewResponse().
            WithMsgKey(consts.RespSuccess).
            WithData(dto.RolesToResponse(dr)).
