...
func CreateAll(cfg Configuration) {
	...
	generateRepo(sc)
	generateDTO(sc)
	createUseCaseList(sc.Column, wk, args[0])
	createUseCaseUpdater(sc.Column, wk, args[0])
	createUseCaseStorer(sc.Column, sc.ImportPackage, wk, args[0])
}
...
func AllTables(cfg Configuration) {
	...
	for i := 0; i < len(tables); i++ {
		...
		createUseCaseList(sc.Column, fmt.Sprintf("%s/%s", wk, tables[i]), tables[i])
		createUseCaseUpdater(sc.Column, packageName(fmt.Sprintf("%s/%s", wk, tables[i])), tables[i])
		createUseCaseStorer(sc.Column, sc.ImportPackage, packageName(fmt.Sprintf("%s/%s", wk, tables[i])), tables[i])
	}
}
