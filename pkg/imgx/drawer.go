...
type Drawer interface {
	...
	SetColors(*image.Uniform, *image.Uniform)
...
func (d *drawer) Draw(text string) (img *image.RGBA, err error) {
	...
	if d.Font != nil {
		...
		textHeight := int(c.PointToFixed(d.FontSize) >> 7)
		//pt := freetype.Pt((d.Width-textWidth)/2+d.TextPosHorizontal, (d.Height+textHeight)/2+d.TextPosVertical)

		w := (d.Width-textWidth)/2 + d.TextPosHorizontal
		h := ((d.Height+textHeight)/2 + d.TextPosVertical)
...
func (d *drawer) SetColors(textColor, backgroundColor *image.Uniform) {
